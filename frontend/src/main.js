...
async function checkUrlForGroup() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      circleSelect.setAttribute(
        'aria-label',
        'Select circle - currently ' + group.name
      );
    }
  } catch (e) {
...
function renderCards() {
  ...
  if (prayersList.length === 0) {
    cardsEl.innerHTML =
      '<p class="cards-empty">No prayers yet - submit from the New Prayers screen.</p>';
...
function resetToAllCircles() {
  ...
  circleSelect.setAttribute(
    'aria-label',
    'Select Circle - Current Any Prayer Group'
  );
  passcodeInput.value = '';
}
/* v8 ignore stop */
...
async function handlePasscodeConfirm() {
  ...
  if (code.length < 3 || code.length > 12) {
    passcodeFeedback.textContent =
      'Passcode must be between 3 and 12 characters.';
  ...
  try {
    ...
    if (!isClosed) {
      ...
      form.addEventListener('submit', async (e) => {
        ...
        if (
          !confirm(
            'Are you sure you want to close this prayer? This will notify your intercessors and cannot be undone.'
          )
        ) {
          return;
        }
    ...
    detailContentArea.innerHTML = `
            <div style="text-align: center; padding: 2rem; color: var(--orb-magenta);">
                <p>Failed to load prayer details.</p>
      ...
      .start(
        ...
        async (qrText) => {
          const match = qrText.match(
            /(?:group\/|groupId=)([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})/i
          );
          ...
          try {
            ...
            qrFeedback.textContent =
              'Could not resolve group from scanned QR code.';
