...
export async function fetchPrayers(deviceId) {
  const url = deviceId
    ? `/api/prayers?deviceId=${encodeURIComponent(deviceId)}`
    : '/api/prayers';
...
export async function validatePasscode(passcode) {
  ...
  const res = await fetchSecureData(
    `/api/groups/validate?passcode=${encodeURIComponent(passcode.trim().toUpperCase())}`
  );
