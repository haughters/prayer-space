...
public class AdminController {
    ...
    public ResponseEntity<Map<String, Object>> getStatus(
        ...
        if (token != null && !token.trim().isEmpty()) {
            try {
                ...
                response.put(
                        "groupId",
                        (jwt.getClaim("groupId").asString() != null
                                ? UUID.fromString(jwt.getClaim("groupId").asString())
                                : null));
    ...
    @PostMapping("/setup")
    public ResponseEntity<?> setup(@RequestBody Map<String, String> body, HttpServletResponse response) {
    ...
    public ResponseEntity<?> login(@RequestBody Map<String, String> body, HttpServletResponse response) {
        ...
        if (adminOpt.isEmpty()
                || !passwordEncoder.matches(password, adminOpt.get().getPasswordHash())) {
        ...
        String token =
                jwtUtil.generateToken(admin.getAdminId(), admin.getUsername(), admin.getRole(), admin.getGroupId());
        setJwtCookie(response, token);

        Map<String, Object> respBody = new HashMap<>();
        respBody.put("role", admin.getRole());
        respBody.put("groupId", admin.getGroupId() != null ? admin.getGroupId().toString() : "");
    ...
    public ResponseEntity<List<Map<String, Object>>> getAdmins(
        ...
        List<Map<String, Object>> list = adminRepository.findAll().stream()
                .map(a -> {
    ...
    public ResponseEntity<?> createAdmin(
            @CookieValue(name = COOKIE_NAME, required = false) String token, @RequestBody Map<String, String> body) {
        checkAuth(token, "APP_ADMIN");

        String username = body.get("username");
        String password = body.get("password");
        String role = body.get("role"); // APP_ADMIN or GROUP_ADMIN
        UUID groupId = body.get("groupId") != null && !body.get("groupId").isEmpty()
                ? UUID.fromString(body.get("groupId"))
                : null;
    ...
    public ResponseEntity<?> deleteAdmin(
            @CookieValue(name = COOKIE_NAME, required = false) String token, @PathVariable("adminId") UUID adminId) {
        DecodedJWT auth = checkAuth(token, "APP_ADMIN");

        if (auth.getSubject().equals(adminId.toString())) {
    ...
    public ResponseEntity<Map<String, Object>> getPrayers(
        ...
        if ("GROUP_ADMIN".equals(auth.getClaim("role").asString())) {
            resolvedGroupId = auth.getClaim("groupId").asString() != null
                    ? UUID.fromString(auth.getClaim("groupId").asString())
                    : null;
        }
    ...
    public ResponseEntity<?> createGroup(
            @CookieValue(name = COOKIE_NAME, required = false) String token, @RequestBody GroupDTO dto) {
        checkAuth(token, "APP_ADMIN");
    ...
    public ResponseEntity<?> deleteGroup(
            @CookieValue(name = COOKIE_NAME, required = false) String token, @PathVariable("groupId") UUID groupId) {
        checkAuth(token, "APP_ADMIN");
