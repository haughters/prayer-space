...
public class Admin {
    ...
    public static final TableSchema<Admin> SCHEMA = StaticTableSchema.builder(Admin.class)
            ...
            .addAttribute(
                    String.class,
                    a -> a.name("passwordHash").getter(Admin::getPasswordHash).setter(Admin::setPasswordHash))
            .addAttribute(
                    String.class, a -> a.name("role").getter(Admin::getRole).setter(Admin::setRole))
            .addAttribute(
                    UUID.class, a -> a.name("groupId").getter(Admin::getGroupId).setter(Admin::setGroupId))
            .addAttribute(
                    Instant.class,
                    a -> a.name("createdAt").getter(Admin::getCreatedAt).setter(Admin::setCreatedAt))
            .build();
    ...
    private UUID groupId; // null for APP_ADMIN, uuid for GROUP_ADMIN
