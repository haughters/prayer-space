...
public class IdentityController {
    ...
    public ResponseEntity<DeviceDTO> register(@RequestBody Map<String, String> body, HttpServletResponse response) {
        ...
        Optional<Device> existingDevice = deviceRepository.findById(deviceUuid);
        HttpStatus status;
        Device device;
        ...
        response.addHeader(
                "Set-Cookie",
                String.format(
                        "%s=%s; Max-Age=31536000; Path=/; HttpOnly; Secure; SameSite=Lax", COOKIE_NAME, deviceId));
    ...
    public ResponseEntity<Void> updateSeen(@PathVariable("deviceId") @Pattern(regexp = UUID_REGEX) String deviceId) {
        Optional<Device> existingDevice = deviceRepository.findById(UUID.fromString(deviceId));
    ...
    public ResponseEntity<DeviceDTO> getMe(@CookieValue(name = COOKIE_NAME, required = false) String cookieDeviceId) {
        ...
        Optional<Device> existingDevice = deviceRepository.findById(UUID.fromString(cookieDeviceId));
        if (existingDevice.isEmpty()) {
            return ResponseEntity.notFound().build();
        }
    ...
    public ResponseEntity<?> registerIntercessor(@RequestBody Map<String, String> body, HttpServletResponse response) {
        ...
        final UUID targetGroupId = group.getGroupId();
        boolean isMember =
                members != null && members.stream().anyMatch(m -> m.getGroupId().equals(targetGroupId));

        if (!isMember) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(Map.of("error", "Email is not pre-authorized for this invite code"));
        }

        // 2. Check if account already exists
        if (intercessorAccountRepository.findByEmail(cleanEmail).isPresent()) {
        ...
        response.addHeader(
                "Set-Cookie",
                String.format(
                        "%s=%s; Max-Age=2592000; Path=/; HttpOnly; SameSite=Strict%s",
                        INTERCESSOR_COOKIE_NAME, token, secureFlag));
    ...
    public ResponseEntity<?> loginIntercessor(@RequestBody Map<String, String> body, HttpServletResponse response) {
        ...
        String cleanEmail = email.trim().toLowerCase();
        Optional<IntercessorAccount> optAccount = intercessorAccountRepository.findByEmail(cleanEmail);

        if (optAccount.isEmpty()
                || !passwordEncoder.matches(password, optAccount.get().getPasswordHash())) {
    ...
    public ResponseEntity<?> logoutIntercessor(HttpServletResponse response) {
        ...
        response.addHeader(
                "Set-Cookie",
                String.format(
                        "%s=; Max-Age=0; Path=/; HttpOnly; SameSite=Strict%s", INTERCESSOR_COOKIE_NAME, secureFlag));
