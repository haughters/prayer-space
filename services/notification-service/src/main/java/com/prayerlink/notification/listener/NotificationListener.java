...
public class NotificationListener {
    ...
    public void listenToNotifications(String messageBody) {
        ...
        try {
            ...
            if ("PrayerCreated".equals(detailType) || "PRAYER_CREATED".equals(detailType)) {
                ...
                com.prayerlink.common.event.MemberAddedEvent event =
                        objectMapper.treeToValue(detailNode, com.prayerlink.common.event.MemberAddedEvent.class);
    ...
    private void processPrayerCreated(PrayerCreatedEvent event) {
        UUID assignedGroupId = event.getAssignedGroupId();
        if (assignedGroupId == null) {
            log.warn(
                    "PrayerCreatedEvent has no assignedGroupId, skipping email notification. Prayer ID: {}",
                    event.getPrayerId());
            return;
        }
    ...
    private void processPrayerUpdated(PrayerUpdatedEvent event) {
        ...
        for (GroupMemberDTO member : eligibleMembers) {
            try {
                String trimmedText = prayer.prayerText().length() > 50
                        ? prayer.prayerText().substring(0, 47) + "..."
                        : prayer.prayerText();
                sendUpdatedEmail(member.getEmail(), group.getName(), prayer.prayerText(), event.getUpdateText());
            } catch (Exception e) {
    ...
    private void sendCreatedEmail(String recipientEmail, String groupName, String prayerText, String actionUrl) {
        ...
        String htmlBody =
                "<div style=\"max-width: 600px; margin: 0 auto; font-family: Inter, sans-serif; color: #1a1a2e;\">\n"
                        ...
                        + "  <p>A prayer request has been shared with <strong>"
                        + groupName + "</strong>.</p>\n"
                        ...
                        + "    <a href=\""
                        + actionUrl
                        + "\" style=\"display: inline-block; background: #d4a574; color: #1a1a2e; padding: 16px 40px; border-radius: 9999px; text-decoration: none; font-weight: 600; font-size: 16px;\">\n"
                        + "      View Prayer & Pray 🙏\n"
                        + "    </a>\n"
                        + "    <p style=\"margin-top: 16px; font-size: 14px;\">\n"
                        + "      Or <a href=\""
                        + portalUrl
                        + "\" style=\"color: #d4a574; text-decoration: underline;\">log in to your portal</a> to view all group prayers.\n"
                        + "    </p>\n"
                        + "  </div>\n"
        ...
        String textBody = "A Prayer Request for Your Group\n\n" + "A prayer request has been shared with "
                + groupName + ":\n\n" + "\""
                + prayerText + "\"\n\n" + "To view the prayer and pray for it, visit: "
                + actionUrl + "\n\n" + "Or log in to your Prayer Link portal to view all prayers: "
                + portalUrl + "\n\n" + "---\n"
                + "You're receiving this because you're a member of "
                + groupName + ".\n" + "To unsubscribe, contact your group administrator.";
    ...
    private void sendUpdatedEmail(String recipientEmail, String groupName, String prayerText, String updateText) {
        ...
        String htmlBody =
                "<div style=\"max-width: 600px; margin: 0 auto; font-family: Inter, sans-serif; color: #1a1a2e;\">\n"
                        ...
                        + "  <p>An update has been shared for a prayer request in <strong>"
                        ...
                        + "    Log in to your <a href=\""
                        + portalUrl
                        + "\" style=\"color: #d4a574; text-decoration: underline;\">Prayer Link portal</a> to view all other requests.\n"
                        + "  </p>\n"
                        + "  <hr style=\"border: none; border-top: 1px solid #eee; margin: 32px 0;\">\n"
                        + "  <p style=\"font-size: 12px; color: #888;\">\n"
                        + "    You're receiving this because you're a member of "
        ...
        String textBody = "A Prayer Update\n\n" + "An update has been shared for a prayer request in "
                + groupName + ".\n\n" + "Original Prayer:\n"
                + "\""
                + prayerText + "\"\n\n" + "Update:\n"
                + "\""
                + updateText + "\"\n\n" + "This prayer request has been closed.\n\n"
                + "Log in to your Prayer Link portal to view all other requests: "
                + portalUrl + "\n\n" + "---\n"
                + "You're receiving this because you're a member of "
                + groupName + ".\n" + "To unsubscribe, contact your group administrator.";
    ...
    private void sendEmail(String recipientEmail, String subject, String htmlBody, String textBody, String fromEmail) {
        try {
            SendEmailRequest request = SendEmailRequest.builder()
                    .destination(
                            Destination.builder().toAddresses(recipientEmail).build())
